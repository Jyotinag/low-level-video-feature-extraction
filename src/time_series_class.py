...
def do_k_fold_evaluation(model, video_X, target, fold=10):
    ...
    for train, test in kfold.split(video_X, target):
        ...
        if hyper_parameters["classification"]:
            ...
            loss, accuracy = model.evaluate(x=[video_X[test]], y=target[test].astype(int), batch_size=64, verbose=1)
            print('On Fold %d test loss: %.3f' % (current_fold, loss))
            list_loss.append(loss)  # Loss is universal

            # Make predictions
            actual_cs = target[test].astype(int)  # Ground Truth
...
def train_model():
    ...
    df = pd.read_csv("video_data_all_class.csv")
    video_data = df[['cs','optical_flow', 'hog_features', 'edge_intensity', 'scene_cuts',
